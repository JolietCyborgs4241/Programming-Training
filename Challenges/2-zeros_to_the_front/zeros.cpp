...
zerosToTheFront(int array[], int count) {
	...
	int nextZeroLocation, current;

	nextZeroLocation = 0;		// this is where the next zero will go - starts at the very front

	for (current = 0 ; current < count ; current++) {

		if (array[current] == 0) {
			array[current] = array[nextZeroLocation];
			array[nextZeroLocation] = 0;	// we do't need to swap since we know one value is 0
			nextZeroLocation++;
		}
	}
}
