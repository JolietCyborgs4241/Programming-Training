...
void runTest(const char *name, int array[], int count) {
...
void zerosToTheFront(int array[], int count) {
	...
	bool movesComplete = false;  //Initialize false to start

	while (!movesComplete){  //keep moving zeros to the left until none are done on an iteration
		movesComplete = true;  //set true, a move on an iteration will reset to false
		for (int i = 0; i < count - 1; i++){ //iterate through array and if you have a zero followed by a non-zero, switch them
			if (array[i] != 0 && array[i+1] == 0){
				int temp = array[i];
				array[i] = array[i+1];
				array[i+1] = temp;
				movesComplete = false;  //if you make a swith on an iteration, rest flag
			}
		}
	}
}